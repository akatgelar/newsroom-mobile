...
class PodcastModel {
  ...
  factory PodcastModel.mapping(Map<String, dynamic> object) {
    return PodcastModel(
      id: object['id'],
      title: object['title'],
      slug: object['slug'],
  ...
  static Future<List<PodcastModel>?> getAll(
      int per_page, int page, String sort) async {
    try {
      var url = "https://newsroom.pikiran-rakyat.com/api/podcast";
      ...
      if (resultData.statusCode == 200) {
        ...
        for (int i = 0; i < jsonData['data'].length; i++) {
          datas.add(PodcastModel.mapping(jsonData['data'][i]));
        }
        return datas;
      } else {
        return null;
  ...
  static Future<PodcastModel?> getById(String id) async {
    try {
      var url = "https://newsroom.pikiran-rakyat.com/api//podcast/" + id;
      var urlData = Uri.parse(url);
      var resultData = await http.get(urlData, headers: {
        'Content-Type': 'application/json',
        'Authorization':
      ...
      if (resultData.statusCode == 200) {
        var jsonData = jsonDecode(resultData.body);
        return PodcastModel.mapping(jsonData['data']);
      } else {
        return null;
      }
    } catch (err) {
